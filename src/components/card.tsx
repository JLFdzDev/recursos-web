	...
	return (
		<article className={clsx(
			...
			<a href={resource.url} target="_blank" rel="noreferrer" className="h-full flex flex-col">
				<img src={resource.poster ? resource.poster : '#'} alt={`Poster for ${resource.name}`}
					className={clsx(
				...
				<div className="flex flex-col flex-1 gap-4 p-6 bg-darken/50">
					...
					<ul className="flex flex-1 gap-2 flex-wrap items-end">
						{resource.tags.map(tag => (
							<li key={tag.id}><Tag name={tag.name} className="text-xs text-darken hover:text-darken hover:bg-primary" /></li>
						))}
					</ul>
				</div>
			</a>
